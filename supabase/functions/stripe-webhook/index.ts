...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (!userEmail || !userId) {
        console.error('❌ [stripe-webhook] User information not found in session');
        return new Response(
          JSON.stringify({ error: "User information not found in webhook" }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          }
      ...
      console.log('👤 [stripe-webhook] Processing payment for:', { userEmail, userId });
