...
export const StripeButton: React.FC<StripeButtonProps> = ({ className }) => {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      // ✅ SÉCURITÉ: Utiliser l'URL Supabase via environment variable
      const apiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-checkout-link`;
      
      // IMPORTANTE: Enviar explicitamente o email do cliente
      const requestData = {
        customer_email: user.email // Garantir que o email está sendo enviado
      };
